...
type UserSignTool interface {
	// 用户签到
	UserSign(ctx context.Context, user UserRepository, sign SignConfigEntity, ts time.Time) *UserSignResp
	// 用户在当前时间，当前签到配置是否已经签到了
	UserIsSigned(ctx context.Context, user UserRepository, sign SignConfigEntity, ts time.Time) bool
}
